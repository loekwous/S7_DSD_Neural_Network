...
@block
def blk_clock_divider(clk, en, clk_out, incoming_frequency=50E6 ,target_frequency = 1E5):
...
@block
def tb_blk_clock_divider():
  clk, en, clk_out = [Signal(False) for _ in range(3)]
  incoming_frequency = 50E6
  target_frequency = 12.5E6
  inst = blk_clock_divider(clk, en, clk_out, incoming_frequency=incoming_frequency ,target_frequency = incoming_frequency)
...
def test():
  vcd_path = os.path.join(os.getcwd(), TRACE_LOCATION, tb_blk_clock_divider.__name__ + ".vcd")
  if os.path.exists(vcd_path):
    #print("Removed old vcd file")
    os.remove(vcd_path)
  inst = tb_blk_clock_divider()
...
def convert():
  clk, en, clk_out = [Signal(False) for _ in range(3)]
  inst = blk_clock_divider(clk, en, clk_out, incoming_frequency=12.5E6 ,target_frequency = 48.8E3)
